...
import { listAll, getByColumn, getById, getBySlug, getFirst, createRecord, updateRecord, deleteRecord, } from '../db/mysql';
...
function getTable(entity) {
    ...
    if (!t)
        throw new HTTPException(400, { message: `Unknown entity: ${entity}` });
    return t;
}
...
async function requireCpCrud(c, next) {
...
cp.post('/auth/login', zValidator('json', z.object({ username: z.string(), password: z.string() })), async (c) => {
    try {
        ...
        if (!user)
            user = await getByColumn('users', 'email', body.username);
        const fakeHash = '$2a$12$LJ3m4ys3Lk0TSwHnbfOMiOXPm1QlFZqFOBmH39JcGpGtI7qJkGzS';
        const storedHash = user ? user.password_hash : fakeHash;
        if (!user || !verifyPassword(body.password, storedHash)) {
            if (user) {
                const a = (user.failed_login_attempts || 0) + 1;
                const upd = { failed_login_attempts: a };
                if (a >= 5) {
                    upd.locked_until = new Date(Date.now() + 15 * 60 * 1000).toISOString();
                    await updateRecord('users', user.id, upd);
                    throw new HTTPException(429, { message: 'Account locked' });
                }
                await updateRecord('users', user.id, upd);
            }
        ...
        await updateRecord('users', user.id, { last_login_at: now, failed_login_attempts: 0, locked_until: null });
        ...
        if (rid) {
            const r = await getById('roles', rid);
            if (r) {
                rn = r.name;
                rp = r.permissions || {};
            }
...
cp.post('/auth/refresh', zValidator('json', z.object({ refresh_token: z.string() })), async (c) => {
    ...
    const user = await getById('users', stored.user_id);
    if (!user || user.is_active === false)
        throw new HTTPException(401, { message: 'User not found or inactive' });
    await updateRecord('refresh_tokens', stored.id, { revoked: true });
...
cp.put('/auth/profile', getCurrentUser, zValidator('json', z.object({ username: z.string().optional(), email: z.string().optional(), full_name: z.string().optional(), avatar_url: z.string().optional(), old_password: z.string().optional(), new_password: z.string().optional() })), async (c) => {
    ...
    if (Object.keys(data).length > 0)
        await updateRecord('users', user.id, data);
    if (body.new_password) {
        const pool = (await import('../db/mysql')).getRawPool();
...
cp.get('/settings/:key', async (c) => {
    ...
    if (!ALLOWED_KEYS.has(key))
        throw new HTTPException(400, { message: `Unknown key: ${key}` });
...
cp.get('/:entity', async (c) => {
    ...
    if (['settings', 'contact-info', 'auth', 'upload'].includes(entity))
        return;
...
cp.get('/:entity/:slug', async (c) => {
    ...
    if (['settings', 'contact-info'].includes(entity))
        return;
    const table = getTable(entity);
...
cp.put('/settings/:key', getCurrentUser, zValidator('json', z.object({ value: z.string() })), async (c) => {
    ...
    if (existing)
        return c.json(await updateRecord('site_settings', existing.key, { value: body.value }));
...
cp.put('/contact-info', getCurrentUser, zValidator('json', z.object({ email: z.string().optional(), phone: z.string().optional(), address: z.string().optional(), map_url: z.string().optional() })), async (c) => {
    ...
    if (ex) {
        await updateRecord('contact_info', ex.id, body);
        return c.json(await getById('contact_info', ex.id));
    }
...
for (const entity of Object.keys(TABLES)) {
    ...
    cp.put(`/${entity}/:id`, getCurrentUser, requireCpCrud, async (c) => {
        ...
        if (body.image && old.image && old.image !== body.image)
            deleteUpload(old.image);
        if (body.icon && old.icon && old.icon !== body.icon)
            deleteUpload(old.icon);
        if (old.gallery && body.gallery) {
            for (const url of old.gallery) {
                if (!body.gallery.includes(url))
                    deleteUpload(url);
            }
        }
