...
import { requirePPDBAdmin, requireNotificationAccess } from './middleware'
...
notif.get('/templates', getCurrentUser, requirePPDBAdmin, async (c) => {
  return c.json(await listAll('notification_templates', { order: 'created_at.desc', limit: 100 }))
})

notif.post('/templates', getCurrentUser, requirePPDBAdmin, zValidator('json', z.object({
...
notif.put('/templates/:id', getCurrentUser, requirePPDBAdmin, zValidator('json', z.object({
...
// ═══════ Notifications ═══════
notif.post('/send', getCurrentUser, requirePPDBAdmin, zValidator('json', z.object({
...
notif.get('/history', getCurrentUser, requirePPDBAdmin, async (c) => {
...
notif.get('/calendar/admin', getCurrentUser, requirePPDBAdmin, async (c) => {
  return c.json(await listAll('academic_calendars', { order: 'event_date.desc', limit: 200 }))
})

notif.post('/calendar', getCurrentUser, requirePPDBAdmin, zValidator('json', z.object({
...
notif.put('/calendar/:id', getCurrentUser, requirePPDBAdmin, zValidator('json', z.object({
...
notif.delete('/calendar/:id', getCurrentUser, requirePPDBAdmin, async (c) => {
